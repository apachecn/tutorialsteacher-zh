...
在上面的视图中，我们已经在表格的最后一行添加了`@Html.ValidationSummary(true, "", new { @class = "text-danger" })`。这是为了在网络应用编程接口返回状态不是 200 正常的错误响应时显示错误消息。

请注意，如果网络应用编程接口响应的状态代码不是 200 OK，我们已经在步骤 2 中创建的 StudentController 类的 Index()动作方法中添加了模型错误。
