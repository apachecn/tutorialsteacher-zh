...
当用户点击上述 UI 中的删除链接时，它会向当前 id 参数的学生控制器发送 HTTP Get 请求`http://localhost:64189/student/delete/{id}`。因此，让我们通过使用Web API删除方法来实现删除功能。
...
这样你就可以在 ASP.NET MVC 中使用Web API的删除方法。**