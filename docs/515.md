...
> 原文:[https://www . tutorial stearn . com/MongoDB/MongoDB-shell-commands](https://www.tutorialsteacher.com/mongodb/mongodb-shell-commands)

MongoDB Shell 是连接、配置、查询和使用 MongoDB 数据库的最快方法。它充当 MongoDB 服务器的命令行客户端。

可以通过在命令提示符/终端执行`mongo`或`mongosh`命令来启动 MongoDB Shell。`mongosh`是新的 MongoDB 外壳，比旧的`mongo`外壳多了一些特色。
...
使用`--username`和`--authenticationDatabase`命令行选项连接需要身份验证的数据库。
