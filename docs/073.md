...
在前一章中，我们使用 Visual Studio 创建了我们的第一个 ASP.NET Core 应用程序。Visual Studio 在内部使用该命令行界面来恢复、生成和发布应用程序。其他更高级别的 ide、编辑器和工具都可以使用 CLI 来支持。NET Core 应用程序。
...
因此，我们可以使用。NET Core 命令行界面，用于创建、恢复包、构建、运行和发布不同类型的。NET Core 应用程序。