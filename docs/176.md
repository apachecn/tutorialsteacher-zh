# 为 CRUD 操作创建Web API——第 2 部分:实现`GET`方法

> 原文：<https://www.tutorialsteacher.com/webapi/implement-get-method-in-web-api>

本节是上一节的延续，我们在这一节中为我们的Web API创建了必要的基础设施。

在本节中，我们将在处理 HTTP GET 请求的 Web API 控制器类中实现 Get 动作方法。

根据Web API命名约定，以“获取”开头的动作方法将处理 HTTP 获取请求。我们可以只给它命名为 Get，也可以用任何后缀。让我们添加我们的第一个 Get 动作方法，并将其命名为 GetAllStudents，因为它将返回数据库中的所有学生。遵循适当的命名方法增加了可读性，任何人都可以容易地理解方法的目的。
...
上面的Web API例子将编译没有错误，但是当您执行 HTTP GET 请求时，它将响应以下发现错误的多个操作。

[![](img/4b57fc58c0c4423d8e258433316085f0.png)](../../Content/images/webapi/multiple-action-error.png)

Web API Error



这是因为不能有多个具有相同类型的相同数量参数的动作方法。以上两种动作方法都不包含任何参数。所以Web API不知道对 HTTP GET 请求`http://localhost:64189/api/student`执行哪种方法。
...
[![](img/7abcd848caf7123effa3bb4b837810ed.png)](../../Content/images/webapi/implement-get1.png)
...
[![](img/6a762ff82ecb14897627cf7a89d6fbea.png)](../../Content/images/webapi/implement-get2.png)
...
如上所述，Web API控制器可以包括多个具有不同参数和类型的`GET`方法。
...
现在，上面的Web API将处理以下 HTTP GET 请求。
...
同样，您可以在Web API中实现`GET`方法来处理不同的 HTTP 获取请求。

下图显示了 Fiddler 中的 HTTP GET 请求。

[![](img/ccc100f6ce116278fa70b681efdde9d0.png)](../../Content/images/webapi/implement-get3.png)
...
[![](img/3ee58373031ac103534e865d9565c1e9.png)](../../Content/images/webapi/fiddler-response.png)
