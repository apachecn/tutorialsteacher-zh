...
因此，我们得到了Lambda 表达式:`s => s.Age > 12 && s.Age < 20` ，其中`**s**`是参数，`**=>**`是λ运算符，`**s.Age > 12 && s.Age < 20**`是身体表达式:
...
同样的方式，我们在 VB.Net 得到的Lambda 表达式可以写成如下:
...
*Note:**VB.Net doesn't support lambda operator `=>`* *## 多参数Lambda 表达式
